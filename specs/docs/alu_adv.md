...
| `split(sep,hdr,ftr)` | Splits the current input line into multiple output records at the point where the function is evaluated. Any output already accumulated in the current output buffer is duplicated into each generated record, and the remainder of the spec continues independently for each generated record. This works naturally with `REDO`, so later spec units can process each split result as its own record. If *sep* is specified, it will be used instead of the active field separator. If *hdr* and/or *ftr* are specified, the first *hdr* fields and last *ftr* fields are omitted. Multiple calls to `split()` or `splitw()` in the same branch without an intervening `REDO` are undefined behavior. See the example in [examples.md](examples.md) to understand how this pseudo-function works. | 
| `splitw(sep,hdr,ftr)` | Like `split()`, but splits the current input line into multiple output records by words instead of fields. Any output already accumulated in the current output buffer is duplicated into each generated record, and the remainder of the spec continues independently for each generated record. If *sep* is specified, it will be used instead of the active word separator. If *hdr* and/or *ftr* are specified, the first *hdr* words and last *ftr* words are omitted. Multiple calls to `split()` or `splitw()` in the same branch without an intervening `REDO` are undefined behavior. | 
