...
# This counts and adds up a list of numbers
    ...
    eof
       ...
       /records./     nextword
``` 
Using files also allows you to include comments. The rules for comments are that either the line begins with a hash mark and a space, and then the entire line is a comment; or the last occurrence of a hash mark and a space is also preceded by a space, and that is where the comment starts. A hash mark that is the very last character on the line (with no trailing space) is treated the same as a hash mark followed by a space. 
