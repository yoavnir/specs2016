...
- Any change that makes an unchanged *specification* produce different output — a non-backward-compatible change, as opposed to a new feature — must be recorded in *Appendix D: Behavior Changes* of `specs/docs/guidebook.md`, under a section for the release in question. **Always clear such a change with the user before writing it up**, since the decision of whether it is a fix or a regression is theirs. Appendix D is technical-reference material: state what changed and show a before/after table of example *specifications*. Do not add tutorial prose, and do not explain how to reproduce the old behavior.
